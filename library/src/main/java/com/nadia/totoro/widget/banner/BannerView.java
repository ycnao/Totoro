...
//                sendEmptyMessageDelayed(MSG_LOOP, LOOP_INTERVAL);
	...
	private void initLinearPosition() {
		...
		layoutParams.bottomMargin = getResources().getDimensionPixelSize(R.dimen.dimen_9);
		mLinearPosition.setPadding(getResources().getDimensionPixelSize(R.dimen.dimen_9), 0, 0, 0);
			...
			if (mLinearPosition.getChildCount() != viewSize) {
				...
				for (int i = 0; i < diffCnt; i++) {
					if (needAdd) {
						...
						layoutParams.rightMargin = getResources().getDimensionPixelOffset(R.dimen.dimen_9);
